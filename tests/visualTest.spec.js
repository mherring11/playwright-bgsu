...
test.describe("Visual Comparison Tests", () => {
  ...
  test("Test First Request Info Form Submission", async ({
    ...
    const email = "test@ap.com";
    const phone = "5551234567";
    const zipCode = "12345";
    const programInterest = "BGSU-M-MBA"; // Adjust this if needed
    const hearAboutUs = "Email";

    console.log(`\n🔹 Running test on: ${browserName}`);
    console.log(`🔹 Navigating to: ${stagingUrl}`);
    await page.goto(stagingUrl, { waitUntil: "domcontentloaded" });
    console.log(`✅ Page loaded: ${stagingUrl}`);

    // **Popup and Form Selectors**
    const popupButton = "button.request-info-popup"; // "Request Info" button
    const formContainer = "#gform_wrapper_6"; // The form wrapper
    const submitButton = "#gform_submit_button_6"; // Submit button
    const confirmationSelector = "h1.headline1"; // Confirmation message

    // **Step 1: Click "Request Info" Button to Open Popup**
    console.log("🔹 Clicking 'Request Info' button...");
    await page.waitForSelector(popupButton, {
      state: "visible",
      timeout: 5000,
    });
    await page.click(popupButton);
    console.log("✅ 'Request Info' popup opened.");

    // **Step 2: Wait for the form to load**
    console.log("🔹 Waiting for the form to appear...");
    ...
    await page.locator("#input_6_1").selectOption({ value: programInterest });
    await page.locator("#input_6_2").fill(firstName);
    await page.locator("#input_6_3").fill(lastName);
    await page.locator("#input_6_6").fill(email);
    await page.locator("#input_6_4").fill(phone);
    await page.locator("#input_6_5").fill(zipCode);
    await page.locator("#input_6_7").selectOption({ value: hearAboutUs });
    ...
    console.log(`   🔹 Program of Interest: ${programInterest}`);
    console.log(`   🔹 How did you hear about us: ${hearAboutUs}`);
    ...
    // **Fix: Get the correct confirmation message**
    const confirmationMessages = await page
      .locator(confirmationSelector)
      .allTextContents();
    const confirmationText = confirmationMessages.find((text) =>
      text.includes("Thanks for your submission!")
    );

    if (!confirmationText) {
      throw new Error("❌ Expected confirmation message not found.");
    }

    console.log(`✅ Confirmation received: "${confirmationText.trim()}"`);
    expect(confirmationText.trim()).toBe(confirmationTextExpected);
    console.log("\n🎉 **Form submission verified successfully!**");
  });

  test("Click 'Apply Now' in Body, fill out the form, and verify submission", async ({
    page,
  }) => {
    try {
      const homePageUrl = config.staging.baseUrl;
      const applyPageUrl = `${config.staging.baseUrl.replace(
        /\/$/,
        ""
      )}/apply/`; // Ensure correct URL format
      const confirmationSelector = "h1.headline2";

      // **Apply Now Button Selector (Body)**
      const applyNowSelector = "a.apply-now.button.button-2[href='/apply/']";
      ...
      async function fillAndSubmitForm() {
        console.log("\n📋 **Filling out the Apply Now form...**");
        ...
        // **Ensure Form Fields Are Enabled**
        ...
        console.log(`   🔹 Program of Interest: ${testData.program}`);
        console.log(
          `   🔹 How did you hear about us: ${testData.howDidYouHear}`
        );

        console.log("\n✅ Form fields filled successfully.");

        // **Submit the form**
        console.log("🚀 Submitting the Apply Now form...");
        await Promise.all([
          page.waitForNavigation({ waitUntil: "domcontentloaded" }),
        ...
        const confirmationMessages = await page
          ...
          .allTextContents();
        const confirmationText = confirmationMessages.find((text) =>
          text.includes("Great! Now, take the next step.")
        );
        ...
        console.log("✅ Confirmation message matches expected value.");
      }

      async function clickApplyNowButton() {
        console.log("\n🔹 Clicking 'Apply Now' in the Body section...");

        await page.waitForSelector(applyNowSelector, {
          state: "visible",
          timeout: 10000,
        });
        await page.click(applyNowSelector);

        console.log("🔹 Waiting for navigation to Apply page...");
        await page.waitForURL(applyPageUrl, { timeout: 15000 });

        console.log("✅ Navigated to Apply page successfully.");
      }

      // **Step 1: Navigate to Homepage**
      console.log(`\n🔹 Navigating to homepage: ${homePageUrl}`);
      await page.goto(homePageUrl, { waitUntil: "domcontentloaded" });
      console.log("✅ Homepage loaded successfully.");

      // **Step 2: Click 'Apply Now' from Body & Submit Form**
      await clickApplyNowButton();
  ...
  test("Verify Online Programs and Getting Started Menus - BGSU", async ({
    page,
  }) => {
    try {
      const homePageUrl = config.staging.baseUrl;
      console.log(`\n🔹 Navigating to BGSU homepage: ${homePageUrl}`);
      await page.goto(homePageUrl, { waitUntil: "domcontentloaded" });
      console.log("✅ Homepage loaded successfully.");
      ...
      const verifyMenu = async (menuName, menuSelector, submenuSelector) => {
        ...
        console.log(
          `🔹 Waiting for '${menuName}' submenu to become visible...`
        );
      ...
      await verifyMenu(
        "Online Programs",
        "li#mega-menu-item-148 > a.mega-menu-link",
      ...
      await verifyMenu(
        "Getting Started",
        "li#mega-menu-item-153 > a.mega-menu-link",
        "li#mega-menu-item-153 > ul.mega-sub-menu"
      );
